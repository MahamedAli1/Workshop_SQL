...
SELECT CountryCode, COUNT(*) AS CityCount FROM City GROUP BY CountryCode HAVING CityCount > 200;
#
...
SELECT DISTINCT Language FROM CountryLanguage JOIN City ON CountryLanguage.CountryCode = City.CountryCode WHERE City.Population BETWEEN 400 AND 500;
#
# 14: What are the name(s) of the cities with a population between 500 and 600 people and the language(s) spoken in them
SELECT City.Name AS CityName, CountryLanguage.Language FROM City JOIN CountryLanguage ON City.CountryCode = CountryLanguage.CountryCode WHERE City.Population BETWEEN 500 AND 600;
#
...
SELECT City.Name FROM City JOIN Country ON City.Id = Country.Capital WHERE Country.Region = (SELECT Region FROM Country WHERE Capital = (SELECT Id FROM City WHERE Name = 'Yaren'));
#
...
WHERE CountryCode IN (
    SELECT Code 
    FROM Country 
    WHERE Region = (SELECT Region FROM Country WHERE Capital = (SELECT Id FROM City WHERE Name = 'Riga'))
);
#
